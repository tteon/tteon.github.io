...
- Make scrollable regions like `<pre>` code blocks keyboard accessible by adding `tabindex="0"`.
- Use semantic list tags (`<dl>`, `<dt>`, `<dd>`) for key-value pair grids instead of generic `<div>`/`<span>`/`<strong>` containers for better screen reader announcements.
