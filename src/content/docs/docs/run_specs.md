...
`seocho new` writes a runnable sample: `schema.yaml`, `docs/`,
`seocho.run.yaml`, and a local README. Use `seocho run --init` when you want
only a commented run-spec template and plan to supply your own ontology and
documents.

A working repo example is also available:
...
With a running DozerDB/Neo4j endpoint and your provider key:

```yaml
ontology: ./schema.yaml
documents: ./docs/
graph: ${NEO4J_URI:-bolt://localhost:7687}
database: neo4j                    # choose an existing experiment database
questions:
  - Which companies reported revenue growth?
  - Who is the CEO of Acme?
```

Defaults: `mara/MiniMax-M2.7` for both phases, `guided` enforcement, and
`pipeline` execution mode. A Bolt graph URI is required. If `database` is omitted,
the SDK derives one from the ontology; online preflight checks that exact target.

`documents` can also point at connector JSONL generated by `seocho connect`:
...
| `graph` / `vector` | storage backend | Bolt graph URI required |
...
documents:
  path: ./docs/                     # .txt .md .csv .json .jsonl .pdf, including connector JSONL
  recursive: true

models:
  default: mara/MiniMax-M2.7        # provider/model for both phases
  indexing: mara/MiniMax-M2         # per-phase override → separate client
  query: mara/MiniMax-M2.7

graph: bolt://localhost:7687        # Bolt graph URI required
graph_user: neo4j
...
output:
  dir: runs                         # report lands in runs/<name>-<timestamp>-<id>/
```
...
The run command accepts a Bolt URI or a `graph` mapping with `kind`
(`neo4j` or `dozerdb`) and `uri`. It does not accept embedded graph paths. The optional `vector:` section adds a hybrid-search
...
In sweeps, blank LanceDB `uri` becomes `<sweep>/<variant>/vectors.lancedb`, and a
...
seocho run [CONFIG] [options]
  ...
  --show-rendered   Print rendered template YAML, or the plain YAML spec
  -o, --output DIR  Report directory (default: runs/<name>-<timestamp>-<id>/)
  --no-track        Index all inputs without reading/writing .seocho_index
...
Related first-run command: `seocho new DIR` creates a runnable sample project.
...
`report.md` (human summary) under `output.dir/<name>-<timestamp>-<id>/`. Empty
...
seocho sweep [SWEEP] [options]
  ...
  -o, --output DIR       Sweep root (default: runs/<name>-<timestamp>-<id>/)
...
Each variant receives target names and a separate output directory:

- every variant requires a Bolt graph URI;
...
Variants run sequentially. Target naming does not provision databases or prove
arbitrary-query workspace isolation. Use existing, separate databases for
independent experiments; no graph is automatically cleared or rolled back.

### Sweep artifacts

```
runs/<sweep-name>-<timestamp>-<id>/
